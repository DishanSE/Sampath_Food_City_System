...
# Concrete command for yearly sales analysis of the supermarket network
class YearlySalesAnalysisCommand(Command):
    def execute(self):
...
def monthly_sales_analysis(branch_id):
    ...
    total_sales = sum(branch_sales)
    average_sales = np.mean(branch_sales)
    print(f"Total Sales: {total_sales} LKR")
    print(f"Average Sales: {average_sales} LKR")
...
def all_branches_monthly_sales_analysis():
    ...
    for branch_id, total_sales in monthly_sales.items():
        print(f"Branch ID: {branch_id}, Total Sales: {total_sales} LKR")

# Function for yearly sales analysis of the supermarket network
def yearly_sales_analysis():
    print("\n===== Yearly Sales Analysis - Supermarket Network =====")
    sales_data = load_data(SALES_FILE)
    branches = load_data(BRANCHES_FILE)

    yearly_sales = {branch[0]: 0 for branch in branches}

    for sale in sales_data:
        branch_id = sale[0]
        sale_date = parse_date(sale[3])
        if sale_date.year == datetime.now().year:
            yearly_sales[branch_id] += int(sale[2])

    branch_ids, sales_amounts = zip(*yearly_sales.items())

    plt.figure(figsize=(10, 6))
    plt.bar(branch_ids, sales_amounts)
    plt.title('Yearly Sales Analysis of All Branches')
    plt.xlabel('Branch ID')
    plt.ylabel('Total Sales (LKR)')
    plt.xticks(rotation=45, ha='right')
    plt.tight_layout()
    ...
    for branch_id, total_sales in yearly_sales.items():
        print(f"Branch ID: {branch_id}, Total Sales: {total_sales} LKR")
...
def main():
    ...
    while True:
        ...
        print("8. Yearly Sales Analysis of All Branches")
        print("9. Log out")
